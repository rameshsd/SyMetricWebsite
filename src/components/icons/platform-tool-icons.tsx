
'use client';

import * as React from 'react';

export type IconProps = { className?: string };

// Utility to inject raw SVG exactly as provided
const RawSVG: React.FC<{ svg: string; className?: string }> = ({ svg, className }) => (
    // The wrapping span respects the className for sizing and positioning
    <span
        className={className}
        // Exact SVG preserved, JSX-safe because it's injected as raw HTML
        dangerouslySetInnerHTML={{ __html: svg }}
    />
);

// 1. Identity & Access Management (User + Check)
export const IdentityAccessIcon: React.FC<IconProps> = ({ className }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 64 64"
        fill="none"
        stroke="currentColor"
        strokeWidth={2.5}
        strokeLinecap="round"
        strokeLinejoin="round"
        className={className}
      >
    <circle cx={32} cy={22} r={10} />
    <path d="M20 46c0-8 24-8 24 0" />
    <circle cx={46} cy={46} r={8} />
    <path d="M42 46l2.5 3 5.5-6" />
  </svg>
);

// 2. Organization Management (exact SVG)
export const OrganizationIcon: React.FC<IconProps> = ({ className }) => (
  <RawSVG
    className={className}
    svg={`<svg fill="currentColor" viewBox="-10 -10 120.00 120.00" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" stroke="currentColor" stroke-width="1" transform="rotate(0)matrix(1, 0, 0, 1, 0, 0)"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#CCCCCC" stroke-width="0.6"></g><g id="SVGRepo_iconCarrier"> <g id="jackhammer"></g> <g id="drilling_machine"></g> <g id="hammer"></g> <g id="measuring_tape"></g> <g id="wrench"></g> <g id="saw"></g> <g id="building"> <g> <path d="M35,37h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C34,36.6,34.4,37,35,37z M36,31h4v4h-4V31z"></path> <path d="M47,37h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C46,36.6,46.4,37,47,37z M48,31h4v4h-4V31z"></path> <path d="M59,37h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C58,36.6,58.4,37,59,37z M60,31h4v4h-4V31z"></path> <path d="M35,48h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C34,47.6,34.4,48,35,48z M36,42h4v4h-4V42z"></path> <path d="M47,48h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C46,47.6,46.4,48,47,48z M48,42h4v4h-4V42z"></path> <path d="M59,48h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C58,47.6,58.4,48,59,48z M60,42h4v4h-4V42z"></path> <path d="M35,59h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C34,58.6,34.4,59,35,59z M36,53h4v4h-4V53z"></path> <path d="M47,59h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C46,58.6,46.4,59,47,59z M48,53h4v4h-4V53z"></path> <path d="M59,59h6c0.6,0,1-0.4,1-1v-6c0-0.6-0.4-1-1-1h-6c-0.6,0-1,0.4-1,1v6C58,58.6,58.4,59,59,59z M60,53h4v4h-4V53z"></path> <path d="M94,42v-3h1c0.6,0,1-0.4,1-1v-4c0-0.6-0.4-1-1-1h-1.4l-3.7-6.5C89.7,26.2,89.4,26,89,26H71v-1h1c0.6,0,1-0.4,1-1v-4 c0-0.6-0.4-1-1-1h-1.7L50.5,7.1c-0.3-0.2-0.7-0.2-1,0L29.7,19H28c-0.6,0-1,0.4-1,1v4c0,0.6,0.4,1,1,1h1v1H11 c-0.4,0-0.7,0.2-0.9,0.5L6.4,33H5c-0.6,0-1,0.4-1,1v4c0,0.6,0.4,1,1,1h1v3v42v3H5c-0.6,0-1,0.4-1,1v4c0,0.6,0.4,1,1,1h90 c0.6,0,1-0.4,1-1v-4c0-0.6-0.4-1-1-1h-1v-3V42z M82,83v-5h4v5H82z M92,43v5h-4v-5H92z M86,64v5h-4v-5H86z M82,62v-5h4v5H82z M88,64h4v5h-4V64z M88,62v-5h4v5H88z M88,55v-5h4v5H88z M86,55h-4v-5h4V55z M82,71h4v5h-4V71z M88,71h4v5h-4V71z M86,48h-4v-5h4 V48z M88,78h4v5h-4V78z M94,37h-1H71v-2h23V37z M88.4,28l2.9,5H71v-5H88.4z M50,9.2L66.4,19H33.6L50,9.2z M29,21h42v2h-1H30h-1V21 z M69,25v9v4v49H59V64c0-0.6-0.4-1-1-1H42c-0.6,0-1,0.4-1,1v23H31V38v-4v-9H69z M57,87H43V65h14V87z M11.6,28H29v5H8.7L11.6,28z M6,35h23v2H7H6V35z M18,43v5h-4v-5H18z M8,83v-5h4v5H8z M14,62v-5h4v5H14z M18,64v5h-4v-5H18z M12,62H8v-5h4V62z M12,64v5H8v-5 H12z M12,71v5H8v-5H12z M14,71h4v5h-4V71z M18,55h-4v-5h4V55z M12,55H8v-5h4V55z M14,78h4v5h-4V78z M12,48H8v-5h4V48z M8,85h11 c0.6,0,1-0.4,1-1V42c0-0.6-0.4-1-1-1H8v-2h21v48H8V85z M94,91H6v-2h1h23h12h16h12h23h1V91z M71,87V39h21v2H81c-0.6,0-1,0.4-1,1v42 c0,0.6,0.4,1,1,1h11v2H71z"></path> </g> </g> <g id="wall"></g> <g id="crane"></g> <g id="barrier_sign"></g> <g id="concept"></g> <g id="shovel"></g> <g id="architecture"></g> <g id="safety_helmet"></g> <g id="worker"></g> <g id="teamwork"></g> <g id="roller_brush"></g> <g id="designs"></g> <g id="trolley"></g> <g id="pick_axe"></g> </g></svg>`}
  />
);

// 3. Study Management (exact SVG)
export const StudyIcon: React.FC<IconProps> = ({ className }) => (
  <RawSVG
    className={className}
    svg={`<svg fill="currentColor" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 496 496" xml:space="preserve" stroke="currentColor"><g> <g> <g> <path d="M248,184c-35.288,0-64,28.712-64,64c0,35.288,28.712,64,64,64c35.288,0,64-28.712,64-64C312,212.712,283.288,184,248,184z M248,296c-26.472,0-48-21.528-48-48s21.528-48,48-48s48,21.528,48,48S274.472,296,248,296z"></path> <path d="M448,200c-23.736,0-43.448,17.336-47.28,40H376v-32h-23.424c-0.728-1.88-1.512-3.768-2.36-5.648l16.576-16.576 l-22.632-22.624l17.472-17.472c8.064,5.76,17.672,8.904,27.784,8.904c12.824,0,24.872-4.992,33.936-14.056 c9.072-9.064,14.064-21.12,14.064-33.944c0-12.824-4.992-24.872-14.064-33.944c-9.064-9.064-21.12-14.056-33.936-14.056 c-12.816,0-24.872,4.992-33.936,14.056c-9.072,9.064-14.064,21.12-14.064,33.944c0,10.112,3.144,19.72,8.904,27.784 l-17.472,17.472l-22.624-22.624l-16.584,16.576c-1.872-0.84-3.76-1.632-5.64-2.36V120h-32V95.28c22.664-3.824,40-23.536,40-47.28 c0-26.472-21.528-48-48-48s-48,21.528-48,48c0,23.736,17.336,43.448,40,47.28V120h-32v23.424c-1.88,0.728-3.768,1.52-5.64,2.36 l-16.584-16.576l-22.624,22.624L145.68,134.36c5.76-8.064,8.904-17.672,8.904-27.784c0-12.824-4.992-24.872-14.064-33.944 c-9.064-9.064-21.12-14.056-33.936-14.056c-12.816,0-24.872,4.992-33.936,14.056c-9.072,9.064-14.064,21.12-14.064,33.944 c0,12.824,4.992,24.872,14.064,33.944c9.064,9.064,21.12,14.056,33.936,14.056c10.112,0,19.712-3.144,27.784-8.904l17.472,17.472 l-22.632,22.624l16.576,16.576c-0.84-1.88-1.632-3.768-2.36,5.648H120v32H95.28c-3.824-22.664-23.536-40-47.28-40 c-26.472,0-48,21.528-48,48s21.528,48,48,48c23.736,0,43.448-17.336,47.28-40H120v32h23.424c0.728,1.88,1.512,3.768,2.36,5.648 l-16.576,16.576l22.632,22.624l-17.472,17.472c-8.064-5.76-17.672-8.904-27.784-8.904c-12.824,0-24.872,4.992-33.936,14.056 c-9.072,9.064-14.064,21.12-14.064,33.944c0,12.824,4.992,24.872,14.064,33.944c9.064,9.064,21.12,14.056,33.936,14.056 c12.816,0,24.872-4.992,33.936-14.056c9.072,9.064,14.064-21.12,14.064-33.944c0-10.112-3.144-19.72-8.904-27.784l17.472-17.472 l22.624,22.624L202.36,350.2c1.872,0.84,3.76,1.632,5.64,2.36V376h32v24.72c-22.664-3.824-40,23.536-40,47.28 c0,26.472,21.528,48,48,48s48-21.528,48-48c0-23.736-17.336-43.448-40-47.28V376h32v-23.424c1.88-0.728,3.768-1.512,5.64-2.36 l16.584,16.576l22.624-22.624l17.472,17.472c-5.76,8.064-8.904,17.672-8.904,27.784c0,12.824,4.992,24.872,14.064,33.944 c9.064,9.064,21.12,14.056,33.936,14.056c12.816,0,24.872-4.992,33.936-14.056c9.072-9.064,14.064-21.12,14.064-33.944 c0-12.824-4.992-24.872-14.064-33.944c-9.064-9.064-21.12-14.056-33.936-14.056c-10.112,0-19.712-3.144-27.784,8.904 l-17.472-17.472l22.632-22.624l-16.576-16.576c0.84-1.88,1.632-3.768,2.36-5.648H376v-32h24.72c3.824,22.664,23.536,40,47.28,40 c26.472,0,48-21.528,48-48C496,221.536,474.472,200,448,200z M366.8,83.952c6.048-6.04,14.08-9.368,22.624-9.368 s16.576,3.328,22.624,9.368c6.048,6.048,9.376,14.08,9.376,22.632s-3.328,16.584-9.376,22.632 c-12.096,12.088-33.16,12.088-45.248,0c-6.048-6.048-9.376-14.08-9.376-22.632S360.752,90,366.8,83.952z M129.2,129.208 c-12.088,12.088-33.152,12.088-45.248,0c-6.048-6.048-9.376-14.08-9.376-22.632s3.328-16.584,9.376-22.632 c6.048-6.04,14.08-9.368,22.624-9.368s16.576,3.328,22.624,9.368c6.048,6.048,9.376,14.08,9.376,22.632 S135.248,123.16,129.2,129.208z M48,280c-17.648,0-32-14.352-32-32s14.352-32,32-32s32,14.352,32,32S65.648,280,48,280z M129.2,412.048c-12.096,12.088-33.16,12.088-45.248,0c-6.048-6.048-9.376-14.08-9.376-22.632s3.328-16.584,9.376-22.632 c6.048-6.04,14.08-9.368,22.624-9.368s16.576,3.328,22.624,9.368c6.048,6.048,9.376,14.08,9.376,22.632 S135.248,406,129.2,412.048z M366.8,366.792c6.048-6.04,14.08-9.368,22.624-9.368s16.576,3.328,22.624,9.368 c6.048,6.048,9.376,14.08,9.376,22.632s-3.328,16.584-9.376,22.632c-12.096,12.088-33.16,12.088-45.248,0 c-6.048-6.048-9.376-14.08-9.376-22.632S360.752,372.84,366.8,366.792z M216,48c0-17.648,14.352-32,32-32s32,14.352,32,32 s-14.352,32-32,32S216,65.648,216,48z M280,448c0,17.648-14.352,32-32,32s-32-14.352-32-32s14.352-32,32-32S280,430.352,280,448z M360,272h-18.896l-1.792,5.544c-1.504,4.672-3.488,9.416-5.88,14.104l-2.656,5.184l13.4,13.392l-33.944,33.944l-13.4-13.392 l-5.184,2.656c-4.68,2.4-9.432,4.376-14.104,5.888L272,341.112V360h-48v-18.888l-5.536-1.792 c-4.672-1.512-9.424-3.496-14.104-5.888l-5.184-2.656l-13.4,13.392l-33.944-33.944l13.4-13.392l-2.656-5.184 c-2.4-4.688-4.384-9.432-5.88-14.104l-1.8-5.544H136v-48h18.896l1.792-5.544c1.504-4.672,3.488-9.416,5.88-14.104l2.656-5.184 l-13.4-13.392l33.944-33.944l13.408,13.4l5.176-2.664c4.664-2.392,9.408-4.368,14.104-5.888l5.544-1.792V136h48v18.888 l5.536,1.792c4.696,1.512,9.44,3.496,14.104,5.888l5.176,2.664l13.408-13.4l33.944,33.944l-13.4,13.392l2.656,5.184 c2.4,4.688,4.384,9.432,5.88,14.104l1.8,5.544H360V272z M448,280c-17.648,0-32-14.352-32-32s14.352-32,32-32s32,14.352,32,32 S465.648,280,448,280z"></path> </g> </g> </g>`}
  />
);

// 4. Site Management (exact SVG)
export const SiteIcon: React.FC<IconProps> = ({ className }) => (
  <RawSVG
    className={className}
    svg={`<svg fill="currentColor" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve" width="212px" height="212px" stroke="currentColor"><g> <g> <path d="M502.857,146.286H365.714V9.143c0-5.047-4.096-9.143-9.143-9.143H155.429c-5.047,0-9.143,4.096-9.143,9.143v137.143H9.143 c-5.047,0-9.143,4.096-9.143,9.143v347.429C0,507.904,4.096,512,9.143,512h146.286h64h73.143h64h146.286 c5.047,0,9.143-4.096,9.143-9.143V155.429C512,150.382,507.904,146.286,502.857,146.286z M146.286,493.714h-128V164.571h128 V493.714z M283.429,493.714h-54.857v-73.143h54.857V493.714z M347.429,155.429v338.286h-45.714v-73.143H320v-18.286h-27.429 h-73.143H192v18.286h18.286v73.143h-45.714V155.429V18.286h182.857V155.429z M493.714,493.714h-128V164.571h128V493.714z"></path> </g> </g> <g> <g> <path d="M118.857,182.857H45.714c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h73.143 c5.047,0,9.143-4.096,9.143-9.143V192C128,186.953,123.904,182.857,118.857,182.857z M73.143,219.429H54.857v-18.286h18.286 V219.429z M109.714,219.429H91.429v-18.286h18.286V219.429z"></path> </g> </g> <g> <g> <path d="M118.857,256H45.714c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h73.143 c5.047,0,9.143-4.096,9.143-9.143v-36.571C128,260.096,123.904,256,118.857,256z M73.143,292.571H54.857v-18.286h18.286V292.571z M109.714,292.571H91.429v-18.286h18.286V292.571z"></path> </g> </g> <g> <g> <path d="M118.857,329.143H45.714c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h73.143 c5.047,0,9.143-4.096,9.143-9.143v-36.571C128,333.239,123.904,329.143,118.857,329.143z M73.143,365.714H54.857v-18.286h18.286 V365.714z M109.714,365.714H91.429v-18.286h18.286V365.714z"></path> </g> </g> <g> <g> <path d="M466.286,182.857h-73.143c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h73.143 c5.047,0,9.143-4.096,9.143-9.143V192C475.429,186.953,471.333,182.857,466.286,182.857z M420.571,219.429h-18.286v-18.286h18.286 V219.429z M457.143,219.429h-18.286v-18.286h18.286V219.429z"></path> </g> </g> <g> <g> <path d="M466.286,256h-73.143c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h73.143 c5.047,0,9.143-4.096,9.143-9.143v-36.571C475.429,260.096,471.333,256,466.286,256z M420.571,292.571h-18.286v-18.286h18.286 V292.571z M457.143,292.571h-18.286v-18.286h18.286V292.571z"></path> </g> </g> <g> <g> <path d="M466.286,329.143h-73.143c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h73.143 c5.047,0,9.143-4.096,9.143-9.143v-36.571C475.429,333.239,471.333,329.143,466.286,329.143z M420.571,365.714h-18.286v-18.286 h18.286V365.714z M457.143,365.714h-18.286v-18.286h18.286V365.714z"></path> </g> </g> <g> <g> <path d="M310.857,73.143h-27.429V45.714c0-5.047-4.096-9.143-9.143-9.143h-36.571c-5.047,0-9.143,4.096-9.143,9.143v27.429 h-27.429c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h27.429v27.429 c0,5.047,4.096,9.143,9.143,9.143h36.571c5.047,0,9.143-4.096,9.143-9.143V128h27.429c5.047,0,9.143-4.096,9.143-9.143V82.286 C320,77.239,315.904,73.143,310.857,73.143z M301.714,109.714h-27.429c-5.047,0-9.143,4.096-9.143,9.143v27.429h-18.286v-27.429 c0-5.047-4.096-9.143-9.143-9.143h-27.429V91.429h27.429c5.047,0,9.143-4.096,9.143-9.143V54.857h18.286v27.429 c0,5.047,4.096,9.143,9.143,9.143h27.429V109.714z"></path> </g> </g> <g> <g> <path d="M237.714,182.857h-36.571c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h36.571 c5.047,0,9.143-4.096,9.143-9.143V192C246.857,186.953,242.761,182.857,237.714,182.857z M228.571,219.429h-18.286v-18.286h18.286 V219.429z"></path> </g> </g> <g> <g> <path d="M310.857,182.857h-36.571c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h36.571 c5.047,0,9.143-4.096,9.143-9.143V192C320,186.953,315.904,182.857,310.857,182.857z M301.714,219.429h-18.286v-18.286h18.286 V219.429z"></path> </g> </g> <g> <g> <path d="M237.714,256h-36.571c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h36.571 c5.047,0,9.143-4.096,9.143-9.143v-36.571C246.857,260.096,242.761,256,237.714,256z M228.571,292.571h-18.286v-18.286h18.286 V292.571z"></path> </g> </g> <g> <g> <path d="M310.857,256h-36.571c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h36.571 c5.047,0,9.143-4.096,9.143-9.143v-36.571C320,260.096,315.904,256,310.857,256z M301.714,292.571h-18.286v-18.286h18.286V292.571 z"></path> </g> </g> <g> <g> <path d="M237.714,329.143h-36.571c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h36.571 c5.047,0,9.143-4.096,9.143-9.143v-36.571C246.857,333.239,242.761,329.143,237.714,329.143z M228.571,365.714h-18.286v-18.286 h18.286V365.714z"></path> </g> </g> <g> <g> <path d="M310.857,329.143h-36.571c-5.047,0-9.143,4.096-9.143,9.143v36.571c0,5.047,4.096,9.143,9.143,9.143h36.571 c5.047,0,9.143-4.096,9.143-9.143v-36.571C320,333.239,315.904,329.143,310.857,329.143z M301.714,365.714h-18.286v-18.286h18.286 V365.714z"></path> </g> </g> <g> <g> <rect x="36.571" y="402.286" width="18.286" height="18.286"></rect> </g> </g> <g> <g> <rect x="73.143" y="402.286" width="18.286" height="18.286"></rect> </g> </g> <g> <g> <rect x="109.714" y="402.286" width="18.286" height="18.286"></rect> </g> </g> <g> <g> <rect x="384" y="402.286" width="18.286" height="18.286"></rect> </g> </g> <g> <g> <rect x="420.571" y="402.286" width="18.286" height="18.286"></rect> </g> </g> <g> <g> <rect x="457.143" y="402.286" width="18.286" height="18.286"></rect> </g> </g> `}
  />
);

// 5. Clinical Supplies (exact SVG)
export const SuppliesIcon: React.FC<IconProps> = ({ className }) => (
  <RawSVG
    className={className}
    svg={`<svg fill="#a73bb5" viewBox="0 0 24 24" id="capsule-pills" data-name="Line Color" xmlns="http://www.w3.org/2000/svg" class="icon line-color" transform="matrix(-1, 0, 0, 1, 0, 0)" stroke="#a73bb5"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path id="primary" d="M13.51,19a4,4,0,0,0,4.61,1.9C19.87,20.37,21,18.68,21,16V7.15a4.39,4.39,0,0,0-2.79-4A4,4,0,0,0,13,7v3" style="fill: none; stroke: #bb1ad1; stroke-linecap: round; stroke-linejoin: round; stroke-width: 2;"></path><line id="primary-2" data-name="primary" x1="14.32" y1="12" x2="21" y2="12" style="fill: none; stroke: #bb1ad1; stroke-linecap: round; stroke-linejoin: round; stroke-width: 2;"></line><path id="secondary" d="M9,9a6,6,0,1,1-6,6A6,6,0,0,1,9,9ZM5,11l8,8" style="fill: none; stroke: #ffffff; stroke-linecap: round; stroke-linejoin: round; stroke-width: 2;"></path></g></svg>`}
  />
);

// 6. Subject Management (exact SVG)
export const SubjectIcon: React.FC<IconProps> = ({ className }) => (
  <RawSVG
    className={className}
    svg={`<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <defs> <style>.cls-1{fill:#a315e5;}.cls-2{fill:#ffffff;}</style> </defs> <g data-name="37. Teamwork" id="_37._Teamwork"> <path class="cls-1" d="M25,26a5,5,0,1,1,5-5A5.006,5.006,0,0,1,25,26Zm0-8a3,3,0,1,0,3,3A3,3,0,0,0,25,18Z"></path> <path class="cls-1" d="M22,32H20a1,1,0,0,1-.98-.8l-1-5a1,1,0,0,1,.533-1.091l3.6-1.8a1,1,0,1,1,.894,1.789L20.13,26.553,20.82,30H22a1,1,0,0,1,0,2Z"></path> <path class="cls-1" d="M30,32H26a1,1,0,0,1,0-2h3.18l.69-3.447L26.953,25.1a1,1,0,1,1,.894-1.789l3.6,1.8A1,1,0,0,1,31.98,26.2l-1,5A1,1,0,0,1,30,32Z"></path> <path class="cls-1" d="M7,26a5,5,0,1,1,5-5A5.006,5.006,0,0,1,7,26Zm0-8a3,3,0,1,0,3,3A3,3,0,0,0,7,18Z"></path> <path class="cls-1" d="M4,32H2a1,1,0,0,1-.98-.8l-1-5a1,1,0,0,1,.533-1.091l3.6-1.8A1,1,0,1,1,5.047,25.1L2.13,26.553,2.82,30H4a1,1,0,0,1,0,2Z"></path> <path class="cls-1" d="M12,32H8a1,1,0,0,1,0-2h3.18l.69-3.447L8.953,25.1a1,1,0,1,1,.894-1.789l3.6,1.8A1,1,0,0,1,13.98,26.2l-1,5A1,1,0,0,1,12,32Z"></path> <path class="cls-2" d="M16.706,16H15.294A2.3,2.3,0,0,1,13,13.706a.294.294,0,0,0-.441-.255,2.294,2.294,0,0,1-3.135-.84l-.7-1.222a2.3,2.3,0,0,1,.84-3.134.295.295,0,0,0,0-.51,2.3,2.3,0,0,1-.84-3.134l.705-1.222a2.294,2.294,0,0,1,3.135-.84A.294.294,0,0,0,13,2.294,2.3,2.3,0,0,1,15.294,0h1.412A2.3,2.3,0,0,1,19,2.294a.284.284,0,0,0,.147.255.291.291,0,0,0,.294,0,2.294,2.294,0,0,1,3.135.84l.7,1.222a2.3,2.3,0,0,1-.84,3.134.295.295,0,0,0,0,.51,2.3,2.3,0,0,1,.84,3.134l-.7,1.222a2.294,2.294,0,0,1-3.135.84.288.288,0,0,0-.294,0,.284.284,0,0,0-.147.255A2.3,2.3,0,0,1,16.706,16Zm-4-4.592A2.3,2.3,0,0,1,15,13.706a.294.294,0,0,0,.294.294h1.412A.294.294,0,0,0,17,13.706a2.294,2.294,0,0,1,3.441-1.987.281.281,0,0,0,.224.029.285.285,0,0,0,.178-.137l.706-1.222a.293.293,0,0,0-.108-.4,2.295,2.295,0,0,1,0-3.974.293.293,0,0,0,.108-.4h0l-.706-1.222a.285.285,0,0,0-.178-.137.289.289,0,0,0-.224.029A2.294,2.294,0,0,1,17,2.294.294.294,0,0,0,16.706,2H15.294A.294.294,0,0,0,15,2.294a2.294,2.294,0,0,1-3.441,1.987.289.289,0,0,0-.224-.029.285.285,0,0,0-.178.137l-.706,1.222a.293.293,0,0,0,.108.4,2.295,2.295,0,0,1,0,3.974.293.293,0,0,0-.108.4l.706,1.222a.285.285,0,0,0,.178.137.28.28,0,0,0,.224-.029A2.292,2.292,0,0,1,12.706,11.408Z"></path> <path class="cls-2" d="M16,10a2,2,0,1,1,2-2A2,2,0,0,1,16,10Zm0-2h0Zm0,0h0Zm0,0h0Zm0,0h0Zm0,0h0Zm0,0h0Zm0,0h0Zm0,0h0Z"></path> </g> </g></svg>`}
  />
);

// 7. Data Management (Hierarchy)
export const DataManagementIcon: React.FC<IconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 64 64"
    fill="none"
    stroke="currentColor"
    strokeWidth={2.5}
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <rect x={10} y={12} width={14} height={10} rx={2} />
    <rect x={40} y={12} width={14} height={10} rx={2} />
    <rect x={25} y={42} width={14} height={10} rx={2} />
    <path d="M17 22v10h30V22" />
    <path d="M32 32v10" />
  </svg>
);

// 8. Lab Management (Microscope)
export const LabIcon: React.FC<IconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 64 64"
    fill="none"
    stroke="currentColor"
    strokeWidth={2.5}
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M24 16v14l4 6h12l4-6" />
    <path d="M28 30h16" />
    <path d="M18 50h28" />
  </svg>
);

// 9. Medical Coding (Shield + Cross)
export const MedicalCodingIcon: React.FC<IconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 64 64"
    fill="none"
    stroke="currentColor"
    strokeWidth={2.5}
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M32 10l18 8v14c0 14-10 22-18 24-8-2-18-10-18-24V18z" />
    <path d="M28 26h8" />
    <path d="M32 22v8" />
  </svg>
);

// 10. Data Services (Cloud + Nodes)
export const DataServicesIcon: React.FC<IconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 64 64"
    fill="none"
    stroke="currentColor"
    strokeWidth={2.5}
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M22 34a10 10 0 1 1 20 0 8 8 0 1 1-4 15H26a8 8 0 1 1-4-15" />
    <circle cx={22} cy={50} r={3} />
    <circle cx={32} cy={50} r={3} />
    <circle cx={42} cy={50} r={3} />
  </svg>
);

// 11. Digital Learning (Graduation Cap)
export const DigitalLearningIcon: React.FC<IconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 64 64"
    fill="none"
    stroke="currentColor"
    strokeWidth={2.5}
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M10 26l22-10 22 10-22 10z" />
    <path d="M32 36v14" />
    <circle cx={44} cy={32} r={2} />
    <path d="M44 34v8" />
  </svg>
);

// 12. Help & Support (Circle + ?)
export const HelpSupportIcon: React.FC<IconProps> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 64 64"
    fill="none"
    stroke="currentColor"
    strokeWidth={2.5}
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <circle cx={32} cy={32} r={24} />
    <path d="M28 24c0-6 8-6 8 0 0 6-8 6-8 12" />
    <circle cx={32} cy={46} r={2} />
  </svg>
);
